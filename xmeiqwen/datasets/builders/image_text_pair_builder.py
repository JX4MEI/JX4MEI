...
from xmeiqwen.datasets.builders.base_dataset_builder import BaseDatasetBuilder
from xmeiqwen.datasets.datasets.mer2025ov_dataset import MER2025OV_Dataset
from xmeiqwen.datasets.datasets.mercaptionplus_dataset import MERCaptionPlus_Dataset
from xmeiqwen.datasets.datasets.ovmerd_dataset import OVMERD_Dataset
from xmeiqwen.datasets.datasets.mer2023 import MER2023_Dataset
from xmeiqwen.datasets.datasets.mer2024 import MER2024_Dataset
from xmeiqwen.datasets.datasets.meld import MELD_Dataset
from xmeiqwen.datasets.datasets.cmumosi  import CMUMOSI_Dataset
from xmeiqwen.datasets.datasets.cmumosei import CMUMOSEI_Dataset
from xmeiqwen.datasets.datasets.sims import SIMS_Dataset
from xmeiqwen.datasets.datasets.simsv2 import SIMSv2_Dataset
from xmeiqwen.datasets.datasets.iemocap import IEMOCAPFour_Dataset
from xmeiqwen.datasets.datasets.ovmerdplus_dataset import OVMERDPlus_Dataset
import logging
from xmeiqwen.common.registry import registry
from xmeiqwen.datasets.builders.base_dataset_builder import BaseDatasetBuilder
from xmeiqwen.datasets.datasets.xmei_dataset import XMEI_Dataset 
...
@registry.register_builder("xmeidataset")
class XMEI_Builder(BaseDatasetBuilder):

    train_dataset_cls = XMEI_Dataset
    eval_dataset_cls = XMEI_Dataset  

    def build_datasets(self):
        logging.info("Building datasets: XMEI_Dataset")
        
        self.build_processors()

        datasets = dict()

        if self.train_dataset_cls:
            datasets['train'] = self.train_dataset_cls(
                vis_processor=self.vis_processors["train"],
                txt_processor=self.txt_processors["train"],
                img_processor=self.img_processors.get("train", None), 
                dataset_cfg=self.dataset_cfg,
                model_cfg=self.model_cfg,
                split="train"  
            )

        if self.eval_dataset_cls:
            vis_proc_val = self.vis_processors.get("eval", self.vis_processors.get("train"))
            txt_proc_val = self.txt_processors.get("eval", self.txt_processors.get("train"))
            img_proc_val = self.img_processors.get("eval", self.img_processors.get("train"))

            datasets['val'] = self.eval_dataset_cls(
                vis_processor=vis_proc_val,
                txt_processor=txt_proc_val,
                img_processor=img_proc_val,
                dataset_cfg=self.dataset_cfg,
                model_cfg=self.model_cfg,
                split="val" 
            )
        ...
        if self.eval_dataset_cls:
            vis_proc_test = self.vis_processors.get("eval", self.vis_processors.get("train"))
            txt_proc_test = self.txt_processors.get("eval", self.txt_processors.get("train"))
            img_proc_test = self.img_processors.get("eval", self.img_processors.get("train"))

            datasets['test'] = self.eval_dataset_cls(
                vis_processor=vis_proc_test,
                txt_processor=txt_proc_test,
                img_processor=img_proc_test,
                dataset_cfg=self.dataset_cfg,
                model_cfg=self.model_cfg,
                split="test" 
            )
